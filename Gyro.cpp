...
void Gyro::setupwire()
{
  ...
  Wire.write(0x08);              // setting it to FS_SEL = 1(0x08) with 65.5 LSB
  ...
  Wire.write(0x10);              // setting it to AFS_SEL to 0x10=4096LSB/g
  Wire.endTransmission();        // end
...
{
  if (countTime) {
    // If 'time per iteration' is not set, count time between iterations
...
void Gyro::calculateAngle()
{
  ...
  // Set acc_angle absolute on first iteration or calculating Angle based on 99% Gyro and 1% Acceleration Angle
  if (GyroSet) {
